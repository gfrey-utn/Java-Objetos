...
public class Table <E> {
    public void cargar(JTable tbl, List<E> list){
        ...
        tbl.setModel(dtm);  // Con esto, limpio la tabla (porque estoy impactando
                            // los datos de una tabla recién creada/vacía)
        if (list==null || list.isEmpty()) return;
        E e = list.get(0);  // Pido el primer elemento de la lista
        Field[] campos = e.getClass().getDeclaredFields();
        // Obtengo todos los atributos de la lista.
        // Luego, por cada atributo, creo una columna con el nombre del atributo.
        for(Field c:campos){
